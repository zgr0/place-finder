...
describe('Backend API Endpoints', () => {
    ...
    describe('GET /venues', () => {
        it('should return 200 and mocked venues upon success', async () => {
            ...
            parser_1.parseCafeGeoJson.mockReturnValue(mockVenues);
            const res = await (0, supertest_1.default)(index_1.default).get('/venues');
            expect(res.status).toBe(200);
            expect(res.body).toEqual(mockVenues);
            expect(parser_1.parseCafeGeoJson).toHaveBeenCalledTimes(1);
        });
        it('should return 500 when parser fails', async () => {
            parser_1.parseCafeGeoJson.mockImplementation(() => {
    ...
    describe('GET /auth/register', () => {
        it('should return 405 Method Not Allowed', async () => {
            const res = await (0, supertest_1.default)(index_1.default).get('/auth/register');
    ...
    describe('POST /auth/register', () => {
        it('should register a new user successfully, normalizing the email', async () => {
            ...
            expect(bcrypt_1.default.hash).toHaveBeenCalledWith('password_123', 10);
        ...
        it('should return 400 if required fields are missing', async () => {
            const res = await (0, supertest_1.default)(index_1.default).post('/auth/register').send({
        ...
        it('should return 500 on database failure', async () => {
            ...
            bcrypt_1.default.hash.mockResolvedValue('mockHash');
            const res = await (0, supertest_1.default)(index_1.default)
                .post('/auth/register')
    ...
    describe('POST /auth/login', () => {
        it('should log in successfully when credentials match', async () => {
            ...
            expect(bcrypt_1.default.compare).toHaveBeenCalledWith('password_123', 'hashedPassword123');
        });
        it('should return 401 on non-existent user email', async () => {
            mockPrismaUser.findUnique.mockResolvedValue(null);
            const res = await (0, supertest_1.default)(index_1.default).post('/auth/login').send({ email: 'no_exist@x.com', password: 'y' });
        ...
        it('should return 401 on incorrect password comparison', async () => {
            ...
            const res = await (0, supertest_1.default)(index_1.default).post('/auth/login').send({ email: 'x@x.com', password: 'wrong' });
            expect(res.status).toBe(401);
            expect(res.body).toEqual({ error: 'Invalid credentials' });
        });
        it('should return 400 if fields are missing', async () => {
            const res = await (0, supertest_1.default)(index_1.default).post('/auth/login').send({ email: 'x@x.com' });
            expect(res.status).toBe(400);
    ...
    describe('POST /territory/ownership', () => {
        ...
        it('should handle tiebreak implicitly by later enum in object iteration, or standard max finding logic', async () => {
            ...
            mockPrismaVenue.findMany.mockResolvedValue(mockVenues);
            const res = await (0, supertest_1.default)(index_1.default).post('/territory/ownership').send({ hexIds: ['hexTie'] });
            expect(res.status).toBe(200);
        ...
        it('should return 400 if hexIds array is missing', async () => {
            const res = await (0, supertest_1.default)(index_1.default).post('/territory/ownership').send({});
            expect(res.status).toBe(400);
        ...
        it('should return 500 on database error during calculation', async () => {
            ...
            const res = await (0, supertest_1.default)(index_1.default).post('/territory/ownership').send({ hexIds: ['hex1'] });
            expect(res.status).toBe(500);
