...
async function main() {
  ...
  // Create venues
  console.log('\nCreating venues...');
  const createdVenues: { id: number; h3Index: string }[] = [];
  for (const [name, category, lat, lng] of VENUES) {
    ...
    const venue = await prisma.$executeRawUnsafe(
      `INSERT INTO "Venue" ("name","category","h3Index","location") VALUES ($1,$2,$3,'') RETURNING id`,
      name, category, h3Index
    );
    // Re-fetch to get id
